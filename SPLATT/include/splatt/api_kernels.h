...
/**
* @brief Tensor times Matrix (TTM) chain. Multiplies every mode of a sparse
*        tensor stored in CSF by the supplied sparse matrices in CSR. The CSR
*        matrices should have as many rows as the corresponding dimension of
*        the tensor.
*
*        This is equivalent to the Tensor Toolbox function:
*
...
*        ** NOTE **
*
*        The output is a dense tensor and has dimensions (ncolumns[0] x ...)
*        with a "row major" layout. The last mode has stride 1, the second to
*        last mode has stride ncolumns[nmodes-1], and so on. For example, if
*        the output is of size F1 x F2 x F3, entry G(i,j,k) is found at
*        tenout[(i * F2 * F3) + (j * F3) + k].
*
*
* @param ncolumns Then number of columns in each matrix.
