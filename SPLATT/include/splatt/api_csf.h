...
/**
\defgroup api_csr_list List of functions for CSR matrices.
@{
*/

/**
* @brief Read a matrix from a file and convert to CSR format.
*
* @param fname The filename to read from.
* @param[out] matrix The CSR matrix.
* @param options An options array allocated by splatt_default_opts().
*
* @return SPLATT error code (splatt_error_t). SPLATT_SUCCESS on success.
*/
...
/**
* @brief Free all memory allocated for a matrix in CSR form.
*
* @param tensors The matrix to free.
* @param options The options array used when forming the matrix.
*/
void splatt_free_csr(
    splatt_csr * matrix,
    double const * const options);
