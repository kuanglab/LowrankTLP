  ...
  idx_t const mode)
{
  ...
  mutex_pool * mutex = mutex_alloc();

  #pragma omp parallel
  {
    val_t * accum = splatt_malloc(nfactors * sizeof(val_t));
    ...
    for(idx_t n=0; n < tt->nnz; ++n) {
      ...
      /* write to output */
      idx_t const out_rowid = tt->ind[mode][n];
      mutex_set_lock(mutex, out_rowid);
      val_t * const restrict outrow = outmat + (out_rowid * nfactors);
      for(idx_t f=0; f < nfactors; ++f) {
        outrow[f] += accum[f];
      }
      mutex_unset_lock(mutex, out_rowid);
    }
