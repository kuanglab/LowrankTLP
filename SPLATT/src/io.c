...
{
  /* load it into a 2-way tensor */
  sptensor_t * tt = tt_read_file(fname);
  if(tt->nmodes != 2) {
    fprintf(stderr, "SPLATT: expected matrix in file '%s'.\n", fname);
    tt_free(tt);
    *matrix = NULL;
    return SPLATT_ERROR_BADINPUT;
  }
  ...
  spmatrix_t * csr = spmat_alloc(tt->dims[0], tt->dims[1], tt->nnz);
  par_memcpy(csr->colind, tt->ind[1], tt->nnz * sizeof(*csr->colind));
  par_memcpy(csr->vals, tt->vals, tt->nnz * sizeof(*csr->vals));

  /* initialize rowptr to later find empty rows */
  #pragma omp parallel for
  for(idx_t i=0; i <= csr->I; ++i) {
    csr->rowptr[i] = SPLATT_IDX_MAX;
  }

  csr->rowptr[0] = 0;
  csr->rowptr[csr->I] = csr->nnz;
  for(idx_t x=0; x < csr->nnz-1; ++x) {
    /* Mark the end of row. */
    if(tt->ind[0][x] != tt->ind[0][x+1]) {
      idx_t const id = tt->ind[0][x];
      csr->rowptr[id + 1] = x+1;
    }
  }

  /* now account for missing rows */
  #pragma omp parallel for
  for(idx_t i=1; i < csr->I; ++i) {
    if(csr->rowptr[i] == SPLATT_IDX_MAX) {
  ...
  FILE * fout)
{
  ...
  for(idx_t n=0; n < tt->nnz; ++n) {
    ...
    fprintf(fout, "%0.20"SPLATT_PF_VAL"\n", tt->vals[n]);
  }
  timer_stop(&timers[TIMER_IO]);
}
